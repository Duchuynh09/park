...
exports.findOneUser = async (req, res, next) => {
  ...
  try {
    ...
    return next(new ApiError(500, "Đã xảy ra lỗi khi truy xuất danh bạ"));
  }
};
exports.updateUser = async (_req, res, next) => {
  if (!_req.body?.name) {
    return next(new ApiError(400, "Name rỗng"));
  }
  try {
    const contactservice = new ContactService(MongoDB.client, "user");
    const document = await contactservice.updateUser(_req.params.id, _req.body);
    return res.send(document);
  } catch (error) {
    return next(
      new ApiError(500, "Đã xảy ra lỗi khi tạo liên hệ tại contact.service")
...
exports.updateArea = async (req, res, next) => {
  try {
    const contactService = new ContactService(MongoDB.client, "Area");
    const document = await contactService.updateArea(
    ...
    return next(
      new ApiError(500, `Loi khi dang cap nhat voi id=${req.params.id}`)
    );
  }
};
